...
% 			disp([i-1,(i-1)*width,errorNum,errorNum4,errorNum-temp,errorNum4-temp4]);
	...
	%                 disp([i,j,d,upper,down,ud,numL_UD,numR_UD]);
					...
					for lr=-left:right
						dL = double(abs( L(i+ud,j+d+lr,1)-R(i+ud,j+lr,1)) );
						da = double(abs( L(i+ud,j+d+lr,2)-R(i+ud,j+lr,2)) );
						db = double(abs( L(i+ud,j+d+lr,3)-R(i+ud,j+lr,3)) );
